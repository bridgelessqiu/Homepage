...
The revelation principle says that, if there is a mechanism $M$ where each player has a dominant strategy, regardless of their private valuations, then **there exists** a mechanism $M'$ derived from $M$ which is DSIC. 

<div style={{borderLeft: "1px solid white", padding: "10px"}}>
**Proof sketch.**

**Note.** A dominant strategy of $i$ can be viewed as the output of a function $s_i(v_i)$, where the input is the valuation (e.g., bidding twice the valuation). 

Given the set of $n$ bidders with private valuation $v_i$, let $s_i(v_i)$ be the dominant strategy of $i$ under the mechanism $M$, $i \in [n]$. That is, under $M$, $s_i(v_i)$ is the bid that $v_i$ would submit using its dominant strategy. 

Consider another mechanism $M'$, where $M'$ takes a bid $b_i$ from each $i$, submit $b = (s_1(b_1), ..., s_n(b_n))$ to $M$, and let $M$ do the rest. 
<span style={{ fontSize: "20px"}}>∎</span>
</div>

#### Remark 
Note that the above is an existence proof (i.e., there exists a $M'$ that somehow knows $s_i$ for all players).