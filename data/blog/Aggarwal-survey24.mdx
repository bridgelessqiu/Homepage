...
As of reading this paper, it is unclear to me an auction is defined at what level. Is it per-query, or a bundle of queries (where each query has a bundle of slots)?
...
Here $C$ is a set of parameters that captures different variants of Budget and RoS constraints. One example can be $C = \{c_1, c_2\}$ where $B_i^{c_1} = B_i$, $w_{ij}^{c_1} = 0$ (i.e., the budget constrain) and $B_i^{c_2} = 0$, $w_{ij}^{c_2} = \tau_i v_{ij}$ (i.e., the RoS constraint).
